...
//	public final String UPLOAD_DIR = new ClassPathResource("static/data/").getFile().getAbsolutePath(); 
	// In my local machine UPLOAD_DIR = C:\Rushabh 19\github
	// upload\File-IO\File_io2\target\classes\static\data
...
//    }
//    
	public FileEntity uploadFileToFileSystem(MultipartFile file) throws IOException {

		String filePath = UPLOAD_DIR + File.separator;
		// filePath =
		// filePath+LocalDate.now().getYear()+File.separator+LocalDate.now().getMonthValue()+File.separator+LocalDate.now().getDayOfMonth();
		filePath = filePath 
				+ LocalDate.now().getYear() + File.separator 
				+ LocalDate.now().getMonth() + File.separator
				+ LocalDate.now().getDayOfMonth();

		// output : C:\\Rushabh 19\\github
		// upload\\File-IO\\File_io2\\target\\classes\\static\\2023\\AUGUST\\9\\side-1.png
		...
		FileEntity fileobj = repo.save(FileEntity.builder()
				.file_name(LocalDate.now() + "$$" + file.getOriginalFilename()) // need to be change
				.file_type(file.getContentType()) // ask which type
				.created_at(LocalDate.now()).modified_at(LocalDate.now()).fileUuid(UUID.randomUUID().toString())
